...
public class Clas17oo18_Ejem4_1 {
	...
	public static void main(String[] args) {
		...
		do {   System.out.println("0.Salir");
		       ...
		       System.out.println("3. Vector profucto por escalar");
		       System.out.println("4. Producto punto de 2 vectores ");
		...
		{         case 1: System.out.println("Tamaño del vector : ");
		          ...
		          case 2:  System.out.println("Tamaño del vector : ");
		                 ...
		                 mostrar(c,T);
		                 break;
		          case 3:  System.out.println("Tamaño del vector : ");
	                     T=entrada.nextInt();
	                     a=new int[T];
	                     generarElementos(a,T);
	                     System.out.println("Vector original : ");
	                     ...
	                     K=entrada.nextInt();
	                     b=new int [T];
	                     productoXescalar(a,b,T,K);
	                     System.out.println("Producto por escalar: ");
	                     mostrar(b,T);
	                     break;
		          case 4:   System.out.println("Tamaño del vector : ");
		                 T=entrada.nextInt();
		                 a=new int[T];
		                 generarElementos(a,T);
		                 b=new int[T];
		                 generarElementos(b,T);
		                 c=new int[T];
		                 R=productoPunto(a,b,T);
		                 mostrar(a,T);
		                 mostrar(b,T);
		                 System.out.println("Producto punto es:  "+R);
		                 break;
		}
		}while(op!=0);
	}
	...
	public static void suma (int X[],int Y[],int Z[],int T ) {
		for(int f=0;f<T;f++) {
			Z[f]=X[f]+Y[f];
		}
	}
	
	public static void productoXescalar (int X[],int Z[],int T,int K ) {
		for(int f=0;f<T;f++) {
			Z[f]=K*X[f];
		}
	}
	
	public static int productoPunto (int X[],int 	Y[],int T ) {
		int PP=0;
		for(int f=0;f<T;f++) {
			PP=PP+X[f]*Y[f];
		}
